...
SillyClient.prototype.connect = function (
  url,
  room_name,
  on_connect,
  on_message,
  ...
  if (typeof WebSocket == 'undefined') {
    alert(
      'Websockets not supported by your browser, consider switching to the latest version of Firefox, Chrome or Safari.'
    )
    return
  }
...
//Process events
SillyClient.prototype.onServerEvent = function (
  author_id,
  cmd,
  data,
  on_message
...
SillyClient.prototype.sendMessage = function (msg, target_ids) {
  ...
  if (target_ids) {
    var target_str =
      '@' +
      (target_ids.constructor === Array ? target_ids.join(',') : target_ids) +
      '|'
    if (msg.constructor === String) msg = target_str + msg
    else throw 'targeted not supported in binary messages'
  }
